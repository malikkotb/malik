...
export default function ParticleDistorter() {
  ...
  useEffect(() => {
    ...
    // Debug UI
    // Remove any existing GUI instances
    const existingGuis = document.querySelectorAll(".lil-gui");
    existingGuis.forEach((gui) => gui.remove());

    const gui = new GUI({ width: 150, title: "Debug UI" });

    // Position the debug UI lower on the screen
    requestAnimationFrame(() => {
      if (gui.domElement) {
        gui.domElement.style.position = "fixed";
        gui.domElement.style.top = "60vh";
      ...
      for (let i = 0; i < parameters.count; i++) {
        ...
        const targetX =
          factor > 0.01
            ...
            (mouseWorldPos.x - originalPositions[i3]) * factor
            : originalPositions[i3];
        const targetZ =
          factor > 0.01
            ? originalPositions[i3 + 2] +
            (mouseWorldPos.z - originalPositions[i3 + 2]) * factor
            : originalPositions[i3 + 2];

        // Calculate spring force
        const springX =
          (targetX - positions[i3]) * parameters.returnSpeed;
        const springZ =
          (targetZ - positions[i3 + 2]) * parameters.returnSpeed;

        // Update velocities with spring force
        velocities[i3] =
          velocities[i3] * parameters.dampening + springX;
        velocities[i3 + 2] =
          velocities[i3 + 2] * parameters.dampening + springZ;
