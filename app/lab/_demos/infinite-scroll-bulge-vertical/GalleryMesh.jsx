...
const vertexShader = `
  ...
  void main() {
    ...
    // Full-height dome: cosine curve spanning entire geometry
    float t = abs(position.y) / 3.0;  // Normalize to 0-1
...
export default function GalleryMesh({ scrollState, config }) {
  ...
  useFrame(() => {
    ...
    const scrollDelta = Math.abs(state.currentY - lastScrollY.current);
    lastScrollY.current = state.currentY;
